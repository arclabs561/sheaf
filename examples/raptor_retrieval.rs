...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let docs: Vec<Doc> = vec![
        doc("Recursive descent parsers build an AST directly.", dim, &[1, 2]),
        doc("Constant folding is a key compiler optimization.", dim, &[0, 3]),
        doc("LR parsers use tables generated from a grammar.", dim, &[1, 0]),
        doc("Loop unrolling reduces branch overhead in compilers.", dim, &[0, 3]),
        doc("Abstract syntax trees represent program structure.", dim, &[2, 0]),
        doc("TCP uses a three-way handshake to open connections.", dim, &[4, 7]),
        doc("Packet loss increases tail latency in data centers.", dim, &[5, 6]),
        doc("Non-blocking sockets allow multiplexed I/O.", dim, &[7, 6]),
        doc("TCP congestion control adapts to bandwidth.", dim, &[4, 5]),
        doc("Socket read timeouts prevent indefinite blocking.", dim, &[7, 6]),
        doc("B-tree indexes speed up range queries.", dim, &[8, 9]),
        doc("MVCC enables snapshot isolation for transactions.", dim, &[10, 9]),
        doc("Schema migrations must preserve compatibility.", dim, &[11, 10]),
        doc("Covering indexes avoid heap lookups for queries.", dim, &[8, 9]),
        doc("Two-phase commit coordinates distributed txns.", dim, &[10, 8]),
        doc("The kernel scheduler assigns threads to cores.", dim, &[12, 14]),
        doc("Syscalls transition from user to kernel space.", dim, &[13, 12]),
        doc("Priority inversion causes deadline misses in RTOS.", dim, &[14, 12]),
        doc("CFS scheduler uses a red-black tree of tasks.", dim, &[14, 12]),
        doc("Syscall overhead matters for high-frequency I/O.", dim, &[13, 6]),
    ];
    let vecs: Vec<Vec<f32>> = docs.iter().map(|(_, v)| v.clone()).collect();

    println!("Building RAPTOR tree: {} docs, dim={}.", docs.len(), dim);
    let config = TreeConfig::new().with_max_depth(3).with_fanout(5).with_min_cluster_size(2);
    let tree = RaptorTree::<Doc, Doc>::build(
        docs, config,
        |ids, fan| greedy_cluster(ids, fan, &vecs),
        |items: &[&Doc]| {
            let text = items.iter().map(|(t, _)| t.as_str()).collect::<Vec<_>>().join(" | ");
            let mut c = vec![0.0f32; dim];
            for (_, v) in items.iter() { for (i, &x) in v.iter().enumerate() { c[i] += x; } }
    ...
    println!("Depth: {} levels, {} nodes total.", tree.depth(), tree.len());
    for lv in 0..tree.depth() {
        println!("  Level {}: {} nodes", lv, tree.get_level(lv).unwrap_or_default().len());
    }
    println!();
    ...
    for &(qtext, qterms) in queries {
        ...
        println!("Query: \"{}\"", qtext);
        for lv in 0..tree.depth() {
            let mut scored: Vec<_> = tree.get_level(lv).unwrap_or_default().iter().filter_map(|n| {
                let (t, v) = n.as_leaf().or_else(|| n.as_summary())?;
                Some((cosine(&qv, v), n.id, t.as_str()))
            }).collect();
            scored.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
            scored.truncate(3);
            let tag = if lv == 0 { "leaf" } else if lv == tree.depth() - 1 { "root" } else { "mid" };
            println!("  Level {} ({}):", lv, tag);
            for (r, &(sim, id, text)) in scored.iter().enumerate() {
                let t = if text.len() > 60 { format!("{}...", &text[..57]) } else { text.to_string() };
...
fn bow(dim: usize, terms: &[(usize, f32)]) -> Vec<f32> {
    ...
    for &(i, w) in terms { if i < dim { v[i] = w; } }
    v
}

fn doc(text: &str, dim: usize, ids: &[usize]) -> Doc {
    (text.into(), bow(dim, &ids.iter().map(|&i| (i, 1.0)).collect::<Vec<_>>()))
}
...
fn greedy_cluster(ids: &[usize], fanout: usize, vecs: &[Vec<f32>]) -> Vec<Vec<usize>> {
    ...
    for &nid in ids {
        let v = match vecs.get(nid) { Some(v) => v, None => continue };
        let (best_i, best_s) = cls.iter().enumerate()
            .filter(|(_, (_, c, _))| *c > 0)
            .map(|(i, (s, c, _))| {
        ...
        if join {
            for (i, &x) in v.iter().enumerate() { cls[best_i].0[i] += x; }
            cls[best_i].1 += 1;
            cls[best_i].2.push(nid);
        } else if cls.len() < cap {
            cls.push((v.clone(), 1, vec![nid]));
        } else if let Some(c) = cls.get_mut(best_i) {
            for (i, &x) in v.iter().enumerate() { c.0[i] += x; }
            c.1 += 1; c.2.push(nid);
        }
    }
    cls.into_iter().map(|(_, _, m)| m).filter(|m| !m.is_empty()).collect()
}
